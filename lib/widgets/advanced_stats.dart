...
class _DurationTrendChartState extends State<DurationTrendChart> {
  ...
  Widget _modeButton(BuildContext context, String label, bool weeklyValue) {
    final colors = AppColorsScope.of(context);
...
class SmartInsights extends StatelessWidget {
    ...
    return Column(
      ...
      children: [
        if (startTimeInsight != null) ...[
          _insightRow(context, Icons.schedule, startTimeInsight),
          const SizedBox(height: 10),
        ],
        if (waterInsight != null)
  ...
  Widget _insightRow(BuildContext context, IconData icon, String text) {
    final colors = AppColorsScope.of(context);
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        ...
        Expanded(
          child: Text(text,
              style:
                  TextStyle(fontSize: 12, height: 1.3, color: colors.textPrimary)),
        ),
      ],
    );
  }
